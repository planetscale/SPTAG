...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        public:
            ...
            Dataset(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_, SizeType capacity_, const void* data_ = nullptr, bool shareOwnership_ = true, int colStart_ = 0, int rowEnd_ = -1)
            {
                Initialize(rows_, cols_, rowsInBlock_, capacity_, data_, shareOwnership_, colStart_, rowEnd_);
            }
            ~Dataset()
            {
            ...
            void Initialize(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_, SizeType capacity_, const void* data_ = nullptr, bool shareOwnership_ = true, int colStart_ = 0, int rowEnd_ = -1)
            {
                if (data != nullptr) {
                    if (ownData) ALIGN_FREE(data);
                    for (char* ptr : incBlocks) ALIGN_FREE(ptr);
