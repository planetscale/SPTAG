  ...
  if (m_pQuantizer) {
    ...
    if (m_options.m_useKV) {
      if (m_options.m_inPlace) {
        m_extraSearcher.reset(new ExtraDynamicSearcher<T>(
            m_options.m_KVPath.c_str(), m_options.m_dim, INT_MAX,
            m_options.m_useDirectIO, m_options.m_latencyLimit,
            m_options.m_mergeThreshold));
      } else {
        m_extraSearcher.reset(new ExtraDynamicSearcher<T>(
            m_options.m_KVPath.c_str(), m_options.m_dim,
            m_options.m_postingPageLimit * PageSize /
                (sizeof(T) * m_options.m_dim + sizeof(int) + sizeof(uint8_t)),
            m_options.m_useDirectIO, m_options.m_latencyLimit,
            m_options.m_mergeThreshold));
      }
    } else {
...
ErrorCode Index<T>::LoadIndexData(
  ...
  if (m_options.m_recovery) {
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Recovery: Loading another in-memory index\n");
    ...
    for (std::string &f : *files) {
      ...
      if (ptr == nullptr ||
          !ptr->Initialize(f.c_str(), std::ios::binary | std::ios::in)) {
    ...
    if (m_options.m_useKV) {
      if (m_options.m_inPlace) {
        m_extraSearcher.reset(new ExtraDynamicSearcher<T>(
            m_options.m_KVPath.c_str(), m_options.m_dim, INT_MAX,
            m_options.m_useDirectIO, m_options.m_latencyLimit,
            m_options.m_mergeThreshold, false, m_options.m_spdkBatchSize,
            m_options.m_bufferLength, m_options.m_recovery));
      } else {
        m_extraSearcher.reset(new ExtraDynamicSearcher<T>(
            m_options.m_KVPath.c_str(), m_options.m_dim,
            m_options.m_postingPageLimit * PageSize /
      ...
      m_extraSearcher.reset(new ExtraDynamicSearcher<T>(
          m_options.m_spdkMappingPath.c_str(), m_options.m_dim,
          m_options.m_postingPageLimit, m_options.m_useDirectIO,
          m_options.m_latencyLimit, m_options.m_mergeThreshold, true,
          m_options.m_spdkBatchSize, m_options.m_bufferLength,
  ...
  if ((m_options.m_useSPDK || m_options.m_useKV) && m_options.m_preReassign) {
    std::shared_ptr<Helper::ReaderOptions> vectorOptions(
        new Helper::ReaderOptions(
            m_options.m_valueType, m_options.m_dim, m_options.m_vectorType,
...
ErrorCode Index<T>::BuildIndexInternal(
  ...
  if (m_options.m_enableSSD) {
    ...
    if (m_pQuantizer) {
      ...
      if (m_options.m_inPlace) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "Currently unsupport SPDK with inplace!\n");
        exit(1);
      } else {
        m_extraSearcher.reset(new ExtraDynamicSearcher<T>(
            m_options.m_spdkMappingPath.c_str(), m_options.m_dim,
    ...
    if (m_extraSearcher != nullptr) {
      ...
      IOBINARY(ptr, ReadBinary,
               sizeof(std::uint64_t) * m_index->GetNumSamples(),
               (char *)(m_vectorTranslateMap.get()));
      if ((m_options.m_useKV || m_options.m_useSPDK) &&
          m_options.m_preReassign) {
...
template <typename T>
ErrorCode Index<T>::AddIndex(const void *p_data, SizeType p_vectorNum,
  ...
  if (m_options.m_distCalcMethod == DistCalcMethod::Cosine && !p_normalized) {
    ...
    for (SizeType i = 0; i < p_vectorNum; i++) {
      COMMON::Utils::Normalize((T *)(vectorSet->GetVector(i)), p_dimension,
                               base);
    }
  } else {
    vectorSet.reset(new BasicVectorSet(
...
ErrorCode Index<T>::DeleteIndex(const void *p_vectors, SizeType p_vectorNum) {
  ...
  if (m_options.m_distCalcMethod == DistCalcMethod::Cosine) {
    ...
    vectorSet.reset(
        new BasicVectorSet(ByteArray((std::uint8_t *)p_vectors,
                                     sizeof(T) * 1 * p_dimension, false),
